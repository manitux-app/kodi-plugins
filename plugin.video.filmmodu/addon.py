...
ADDON_ID = "plugin.video.filmmodu"
ADDON = xbmcaddon.Addon(id=ADDON_ID)
HANDLE = int(sys.argv[1])
BASE_URL = ADDON.getSetting("base_url") or "https://filmmodu.one"
...
def detail(url):
    ...
    for source in info.get("sources", []):
        add_video(
            ...
            info["image"],
            info["plot"],
            {"referer": source.get("referer") or url, "subtitle": source.get("subtitle") or ""},
        )
    if not info.get("sources"):
        xbmcgui.Dialog().notification("FilmModu", "Video kaynagi bulunamadi", xbmcgui.NOTIFICATION_WARNING, 3000)
...
def play_source(url, referer=None, subtitle=""):
...
def run():
    ...
    if not action:
        ...
        play_source(url, params.get("referer"), params.get("subtitle", ""))
